        ...
        <div class="box">
            <form action="" method="post" enctype="multipart/form-data">
                <div class="box" style="display: flex; flex-direction: column; gap: 1rem;">
                    <label for="service_id">Servis Seç:</label>
                    <select name="service_id" required class="box">
                        <option value="">Servis seçiniz</option>
                        <?php while ($service = $select_services->fetch(PDO::FETCH_ASSOC)) { ?>
                            <option value="<?= $service['id']; ?>"><?= $service['name']; ?></option>
                        <?php } ?>
                    </select>
                    
                    <label for="process_detail">Açıklama:</label>
                    <textarea name="process_detail" required placeholder="Yapılan işlem detayı..." class="box" style="resize: vertical; min-height: 100px;"></textarea>
